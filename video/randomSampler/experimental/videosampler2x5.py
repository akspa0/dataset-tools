...
def process_video(video_file, segment_duration, score_threshold, used_segments, max_clips_per_video):
    clips = []
    metadata = []
    ...
    num_segments = min(max_clips_per_video, int(clip_duration // segment_duration))
    attempts = 0

    while len(clips) < num_segments and attempts < num_segments * 2:
...
def create_sampler(input_dir, output_dir, duration, max_videos, segment_duration, filter_string, log_file, score_threshold, max_clips_per_video):
    ...
    for video_file in tqdm(video_files, desc="Processing videos"):
        try:
            result_clips, result_metadata = process_video(video_file, segment_duration, score_threshold, used_segments, max_clips_per_video)
            clips.extend(result_clips)
            metadata.extend(result_metadata)
            total_clips_duration += sum([clip.duration for clip in result_clips])
            print(f"Processed {video_file}: {len(result_clips)} clips")
...
def main():
    ...
    parser.add_argument('--max_clips_per_video', type=int, default=3, help='Maximum number of clips to take from each input video.')
    ...
    create_sampler(input_dir, output_dir, 30, max_videos, segment_duration, filter_string, log_file, score_threshold, max_clips_per_video)
    print("Creating 1-minute sampler video...")
    create_sampler(input_dir, output_dir, 60, max_videos, segment_duration, filter_string, log_file, score_threshold, max_clips_per_video)
